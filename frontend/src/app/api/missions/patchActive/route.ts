...
const handler = async (req: Request) => {
  ...
  const { id, mission_id, program_id, programReserve } = await req.json();
  ...
  try {
    ...
    await connection.query(`UPDATE missions SET is_confirm = 1, mission_id = ? WHERE id = ?;`, [mission_id, id]);

    await connection.query(`UPDATE programs SET reserve = ? WHERE id = ?;`, [programReserve, program_id]);
