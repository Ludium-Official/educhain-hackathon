    ...
    if (mission.category === "announcement") {
      return {
        ...
        submissions: submissions.filter(
          (submission) =>
            submission.mission_id === mission.id && submission.chapter_id === 0
        ),
      };
    } else {
      const missionChapters = chapters.filter(
        (chapter) => chapter.mission_id === mission.id
      );
      return {
        ...mission,
        chapters: missionChapters.map((chapter) => ({
          ...
          submissions: submissions.filter(
            (submission) =>
              submission.mission_id === mission.id &&
